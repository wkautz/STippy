package edu.stanford.wkautz.tippy
...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etSplit.addTextChangedListener(object: TextWatcher {
            override fun afterTextChanged(s: Editable?) {
                Log.i(TAG, "onSplitTextChanged: $s")
                computeTipAndTotal()
            }
    ...
    private fun computeTipAndTotal() {
        ...
        var splitAmount = 1.0
        if (etSplit.text.isNotEmpty()) {
            splitAmount = etSplit.text.toString().toDouble()
        }
        val tipAmount = baseAmount * tipPercent / 100
        val totalAmount = (tipAmount + baseAmount)
        val splitTotalAmount = (tipAmount + baseAmount) / splitAmount
        tvTipAmount.text = "%.2f".format(tipAmount)
        tvTotalAmount.text = "%.2f".format(totalAmount)
        tvPerPersonAmount.text = "%.2f".format(splitTotalAmount)
    }
}